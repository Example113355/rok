    ...
    const getRokData = () => {
        switch (selectedValues[0].value) {
            case 1:
                ...
                break
            case 2:
                data = packages.filter((item) => item.vip_type === '1')
                break
            case 3:
                data = packages.filter((item) => item.vip_type === '2')
                break
            case 4:
                data = packages.filter((item) => item.vip_type === '3')
                break
            case 5:
                data = packages.toSorted((a, b) => {
                ...
                break
            case 6:
                data = packages.toSorted((a, b) => parseInt(a.price.replace(/\./g, '').replace('đ', '')) - parseInt(b.price.replace(/\./g, '').replace('đ', '')))
                break
            case 7:
                data = packages.toSorted((a, b) => parseInt(b.price.replace(/\./g, '').replace('đ', '')) - parseInt(a.price.replace(/\./g, '').replace('đ', '')))
                break
            default:
                data = packages
        }
